...
# There are as many routing tables as the number of NAT gateways (If enable_nat_gateway == false create only one private subnet)
  ...
  tags = merge(
    {
      ...
      # Modified value from old configuration
      "Name" = var.single_nat_gateway || var.enable_nat_gateway == false ? "${var.name}-${var.private_subnet_suffix}" : format("${var.name}-${var.private_subnet_suffix}-%s", split("-", element(var.azs, count.index))[2])
    },
    var.tags,
...
resource "aws_route_table_association" "private" {
  ...
  route_table_id = element(
    ...
    var.single_nat_gateway || var.enable_nat_gateway == false ? 0 : count.index,
  )
}
...
locals {
  ...
  # Modified value from old configuration
  nat_gateway_count = var.single_nat_gateway || var.enable_nat_gateway == false ? 1 : var.one_nat_gateway_per_az ? length(var.azs) : local.max_subnet_length
